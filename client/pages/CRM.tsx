...
export default function CRM() {
  ...
  const totalValue = filteredContacts.reduce((sum, contact) => sum + contact.value, 0);
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-light">{filteredContacts.length}</p>
                  </div>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-light">${totalValue.toLocaleString()}</p>
                  </div>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-light">{Math.round((wonDeals / totalDeals) * 100)}%</p>
                  </div>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-light">{mockContacts.filter(c => !c.stage.includes('closed')).length}</p>
                  </div>
          ...
          <Tabs defaultValue="contacts" className="space-y-6">
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-start space-x-4">
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-sm text-muted-foreground mb-2">{contact.position} at {contact.company}</p>
            ...
            <TabsContent value="pipeline" className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
                {pipelineStages.map((stage) => {
                  const stageContacts = mockContacts.filter(c => c.stage === stage.id);
                  ...
                  return (
                    <Card key={stage.id}>
                      <CardHeader className="pb-3">
                        ...
                        <CardDescription>
                          {stageContacts.length} contacts • ${stageValue.toLocaleString()}
                        </CardDescription>
                      </CardHeader>
                      <CardContent className="pt-0">
                        <div className="space-y-3">
                          {stageContacts.map((contact) => (
                            <div key={contact.id} className="p-3 bg-muted/30 rounded border">
                              <div className="font-medium text-sm">{contact.name}</div>
                              <div className="text-xs text-muted-foreground">{contact.company}</div>
                              <div className="text-xs font-medium mt-1">${contact.value.toLocaleString()}</div>
                            </div>
                          ))}
                        </div>
                      </CardContent>
                ...
                <CardContent>
                  <div className="space-y-4">
                    {recentActivities.map((activity) => (
                      <div key={activity.id} className="flex items-center space-x-3 pb-4 border-b border-border/30 last:border-0">
                        ...
                        <div className="flex-1">
                          <div className="font-medium text-sm">{activity.contact}</div>
